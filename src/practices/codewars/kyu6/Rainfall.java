...
public class Rainfall {
  ...
  private static Optional<DoubleStream> doubleStream (String town, String strng) {
  ...
  public static double mean(String town, String strng) {
    // your code
    return (doubleStream(town, strng)
        .map(x-> x.sum()/12)
        .orElse(-1.0));
  }
  ...
  public static double variance(String town, String strng) {
    // your code
    return (doubleStream(town, strng)
        .map(x-> x.map(d-> Math.pow(d-mean(town, strng), 2)))
        .map(x-> x.sum()/12)
        .orElse(-1.0));
  }
